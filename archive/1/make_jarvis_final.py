...
if os.path.exists(BASE):
    shutil.rmtree(BASE)
os.makedirs(os.path.join(BASE, "Sources", "JarvisServer", "Resources"))

files = {}

files["Package.swift"] = r'''// swift-tools-version:5.9
...
files["Sources/JarvisServer/Config.swift"] = r'''import Foundation
...
files["Sources/JarvisServer/Logger.swift"] = r'''import Foundation
...
files["Sources/JarvisServer/SysMonitor.swift"] = r'''import Foundation
...
files["Sources/JarvisServer/MetalEngine.swift"] = r'''import Foundation
import Metal
    ...
    private func compileShaders() {
        ...
        do {
            ...
            if let fn = library.makeFunction(name: "jarvis_compute") {
                ...
                Logger.shared.write("Metal: shaders compiled OK")
            }
        } catch {
            Logger.shared.write("Metal: \(error.localizedDescription)")
        }
    }
...
files["Sources/JarvisServer/MidiController.swift"] = r'''import Foundation
import CoreMIDI
...
final class MidiController {
    ...
    func setup() {
        ...
        guard status == noErr, let client = client else {
            Logger.shared.write("MIDI: client failed (\(status))")
            return
        }
        status = MIDIOutputPortCreate(client, "JARVIS-Out" as CFString, &outPort)
        if status == noErr {
            Logger.shared.write("MIDI: ready, \(MIDIGetNumberOfDestinations()) destinations")
        } else {
            Logger.shared.write("MIDI: output port failed (\(status))")
        }
    }
}
'''

files["Sources/JarvisServer/WebServer.swift"] = r'''import Foundation
import Network
...
files["Sources/JarvisServer/main.swift"] = r'''import Foundation
...
default:
    ...
    Logger.shared.write("Server ready")
    while true { Thread.sleep(forTimeInterval: 1) }
}
'''

files["Sources/JarvisServer/Resources/index.html"] = r'''<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
if result.returncode != 0:
    ...
    print("\nBUILD OK")
    bpath = os.path.join(BASE, ".build/release/JarvisServer")
    print(f"  Binary: {bpath}")
    print(f"  Run:    {bpath}")
